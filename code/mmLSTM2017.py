...
flags.DEFINE_string("traindir", "./train/2017/", "Directory to save checkpoints and gnuplot files.")
...
resultFile = open("2017/ResultFile_withattribute" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
dataset = "BPIC2017_withattribute.txt"
...
with tf.Graph().as_default() as G:
    ...
    input_data_c2 = tf.placeholder(tf.int64, [FLAGS.batch_size], name="Input_Cluster2")
    ...
    input_c2 = tf.nn.embedding_lookup(embedding_clusters[2], input_data_c2, name="Embedding_Lookup_c2")
    ...
    with tf.variable_scope("Enhance_Representation_e") as scope_name:
        outputs_enhanced_e = []
        for i, (output_a, output_e) in enumerate(zip(outputs_A, outputs_e)):
            if i > 0: scope.reuse_variables()
            outputs_enhanced_e.append(tf.multiply(output_a, output_e))
    ...
    with tf.variable_scope("lstm2") as scope:
        ...
        outputs_2 = []
        states_2 = []
        for i,input_ in enumerate(outputs_enhanced_e):
    ...
    for i in range(FLAGS.num_epochs):
        ...
        for batchNum, (c, x, y) in enumerate(
            ...
            batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, 
                crossEntropys, cost, finalState, train_op], {input_data_c0: c[0], input_data_c1: c[1], input_data_c2: c[2],
                input_data_es[0]: x[0], input_data_es[1]: x[1], input_data_es[2]: x[2], input_data_es[3]: x[3],
                targets[0]:y[0], targets[1]:y[1], targets[2]:y[2],targets[3]:y[3]})
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, crossEntropys, cost, finalState, tf.no_op()], {
            input_data_c0: c[0], input_data_c1: c[1], input_data_c2: c[2],input_data_es[0]: x[0], input_data_es[1]: x[1], input_data_es[2]: x[2],input_data_es[3]: x[3],
            targets[0]:y[0], targets[1]:y[1], targets[2]:y[2], targets[3]:y[3]})
        accumCosts += batchCost
        accumNumCorrPred = plus(accumNumCorrPred, batchNumCorrPred)
        accumCrossEnt = plus(accumCrossEnt, batchCrossEnt)
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2017/SuffixPrediction{0}Dataset.txt".format(dataset), "w")
        ...
        for batchNum in range(numBatches):
            ...
            for i in range(maxSuffixLen):
                batchPredictions, batchsoftmaxes, batchFinalState, _ = sess.run([prediction_indices, softmaxes, finalState, tf.no_op()], {input_data_c0: c[0], input_data_c1: c[1], input_data_c2: c[2],
                input_data_es[0]: x[0], input_data_es[1]: x[1], input_data_es[2]: x[2], input_data_es[3]: x[3],
