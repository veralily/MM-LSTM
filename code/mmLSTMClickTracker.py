...
flags.DEFINE_string("traindir", "./train/ClickTracker", "Directory to save checkpoints and gnuplot files.")
...
resultFile = open("ClickTracker/ResultFile_withattribute" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
dataset = "ClickTracker_withattribute.txt"
...
with tf.Graph().as_default() as G:
    ...
    outputs_e = []
    for i, (output_e0, output_e1, output_e2) in enumerate(zip(outputs_e0, outputs_e1, outputs_e2)):
        if i > 0: scope.reuse_variables()
        output_e = tf.concat([output_e0, output_e1,output_e2], 1)
    ...
    with tf.variable_scope("Modulator_e", reuse=tf.AUTO_REUSE) as scope: 
        inputs_a = tf.get_variable('a', [FLAGS.hidden_size_e0 + FLAGS.hidden_size_e1 +FLAGS.hidden_size_e2], initializer=norm_c, dtype=tf.float32)
        outputs_A = []
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "ClickTracker/SuffixPrediction{0}Dataset.txt".format(dataset), "w")
