...
def _read_words(filename):
  ...
  with open(filename, "r") as f:
    lines = f.readlines()
    length = len(lines)
    print("length_traces: " + str(length))
...
def data_split(data_path=None, input_file=None, shuffle=True):
  num_pieces = 10
...
def extract_cluster_event(trace_list, cluster2Id, event2Id):
  ...
  for trace in trace_list:
    ...
    if ':' in event_list[0]:
      ...
      cluster_names = event_list[0].replace(':', '').split('-')
      event_words = event_list[1:]
      ...
      for word in event_content:
        for i, attr in enumerate(word):
          event_attrs[i].append(event2Id[i][attr]) 
