...
flags.DEFINE_string("traindir", "./train/2012", "Directory to save checkpoints and gnuplot files.")
...
resultFile = open("2012/ResultFile_withattribute" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
dataset = "BPIC2012_withattribute.txt"
...
with tf.Graph().as_default() as G:
    ...
    outputs_e = []
    for i, (output_e0, output_e1) in enumerate(zip(outputs_e0, outputs_e1)):
        if i > 0: scope.reuse_variables()
        output_e = tf.concat([output_e0, output_e1], 1)
    ...
    with tf.variable_scope("modulator_e", reuse=tf.AUTO_REUSE) as scope: 
        inputs_a = tf.get_variable('a', [FLAGS.hidden_size_e0 + FLAGS.hidden_size_e1], initializer=norm_c, dtype=tf.float32)
        outputs_A = []
    ...
    cost = tf.reduce_sum(losses[0]+losses[1], name="SumLoss") / FLAGS.batch_size
    ...
    for i in range(FLAGS.num_epochs):
        ...
        for batchNum, (c, x, y) in enumerate(
            ...
            batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, 
                ...
                input_data_es[0]: x[0], input_data_es[1]: x[1], targets[0]:y[0], targets[1]:y[1]})
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, crossEntropys, cost, finalState, tf.no_op()], {
            input_data_c0: c[0], input_data_c1: c[1],input_data_es[0]: x[0], input_data_es[1]: x[1], 
            targets[0]:y[0], targets[1]:y[1]})
        accumCosts += batchCost
        accumNumCorrPred = plus(accumNumCorrPred, batchNumCorrPred)
        accumCrossEnt = plus(accumCrossEnt, batchCrossEnt)
        iters += FLAGS.num_unroll_step
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2012/SuffixPrediction{0}.txt".format(dataset), "w")
