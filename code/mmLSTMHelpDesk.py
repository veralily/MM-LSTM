...
flags.DEFINE_string("traindir", "./train/HelpDesk", "Directory to save checkpoints and gnuplot files.")
flags.DEFINE_integer("batch_size", 20, "Batch size of training.")
flags.DEFINE_integer("num_unroll_step", 2, "The history length of process.")
...
flags.DEFINE_integer("hidden_size_e1", 50, "Hidden size of lstm nn for e1.")
flags.DEFINE_integer("hidden_size_e2", 50, "Hidden size of lstm nn for e2.")
flags.DEFINE_integer("hidden_size_e3", 50, "Hidden size of lstm nn for e3.")
flags.DEFINE_integer("hidden_size_e", 100, "Hidden size of lstm nn for e.")
...
flags.DEFINE_integer("hidden_size_A", 20, "Hidden size of modulator.")
...
flags.DEFINE_integer("num_epochs_full_lr", 25,
                     "The number of epochs with full learning rate.")
flags.DEFINE_integer("num_epochs", 40, "The exact number of epochs.")
...
resultFile = open("HelpDesk/ResultFile_withattribute" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
dataset = "helpdesk_withattribute.txt"
...
with tf.Graph().as_default() as G:
    ...
    input_data_e0 = tf.placeholder(tf.int64, [FLAGS.batch_size, FLAGS.num_unroll_step], name="Input_Event0")
    input_data_e1 = tf.placeholder(tf.int64, [FLAGS.batch_size, FLAGS.num_unroll_step], name="Input_Event1")
    input_data_e2 = tf.placeholder(tf.int64, [FLAGS.batch_size, FLAGS.num_unroll_step], name="Input_Event2")
    input_data_e3 = tf.placeholder(tf.int64, [FLAGS.batch_size, FLAGS.num_unroll_step], name="Input_Event3")
    ...
    input_e0 = tf.nn.embedding_lookup(embedding_events[0], input_data_e0, name="Embedding_Lookup_e0")
    input_e1 = tf.nn.embedding_lookup(embedding_events[1], input_data_e1, name="Embedding_Lookup_e1")
    input_e2 = tf.nn.embedding_lookup(embedding_events[2], input_data_e2, name="Embedding_Lookup_e2")
    input_e3 = tf.nn.embedding_lookup(embedding_events[3], input_data_e3, name="Embedding_Lookup_e3")
    ...
    if (FLAGS.dropout_prob > 0):
        input_e0 = tf.nn.dropout(input_e0, 1.0-FLAGS.dropout_prob, name="Input_e0_Dropout")
        input_e1 = tf.nn.dropout(input_e1, 1.0-FLAGS.dropout_prob, name="Input_e1_Dropout")
        input_e2 = tf.nn.dropout(input_e2, 1.0-FLAGS.dropout_prob, name="Input_e2_Dropout")
        input_e3 = tf.nn.dropout(input_e3, 1.0-FLAGS.dropout_prob, name="Input_e3_Dropout")
    
    # reshape inputs
    input_e0 = [tf.squeeze(input_, [1]) for input_ in tf.split(input_e0, FLAGS.num_unroll_step, 1)]
    input_e1 = [tf.squeeze(input_, [1]) for input_ in tf.split(input_e1, FLAGS.num_unroll_step, 1)]
    input_e2 = [tf.squeeze(input_, [1]) for input_ in tf.split(input_e2, FLAGS.num_unroll_step, 1)]
    input_e3 = [tf.squeeze(input_, [1]) for input_ in tf.split(input_e3, FLAGS.num_unroll_step, 1)]
    with tf.variable_scope("lstm_event_e0") as scope:
        ...
        outputs_e0 = []
        state = initialState
        for i,input_ in enumerate(input_e0):
    ...
    with tf.variable_scope("lstm_event_e1") as scope:
        ...
        outputs_e1 = []
        state = initialState
        for i,input_ in enumerate(input_e1):
            if i > 0: scope.reuse_variables()            
    ...
    with tf.variable_scope("lstm_event_e2") as scope:
        ...
        outputs_e2 = []
        state = initialState
        for i,input_ in enumerate(input_e2):
            if i > 0: scope.reuse_variables()            
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_e1,
                                scope_name='input_layer', reuse_scope=(i!=0)))
    ...
    with tf.variable_scope("lstm_event_e3") as scope:
        # Create the individual LSTM layers
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_e3, forget_bias=0.0)
        # Add a propabilistic dropout to cells of the LSTM layer
        if (FLAGS.dropout_prob > 0):
            lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=1.0-FLAGS.dropout_prob)
        # Replicate this (including dropout) to additional layers
        ...
        outputs_e3 = []
        state = initialState
        for i,input_ in enumerate(input_e3):
            if i > 0: scope.reuse_variables()            
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_e1,
                                scope_name='input_layer', reuse_scope=(i!=0)))
            ...
            output, state = cell(input_, state)
            print("ouput.shape,{}".format(output.get_shape()))
            outputs_e3.append(output)

        # connect inputs and outputs to the RNN layers
        print("----------outpute3-------------")
        print(outputs_e3[0].get_shape())
        print("------------------------------------")


    outputs_e = []
    for i, (output_e0, output_e1, output_e2, output_e3) in enumerate(zip(outputs_e0, outputs_e1, outputs_e2, outputs_e3)):
        if i > 0: scope.reuse_variables()
        output_e = tf.concat([output_e0, output_e1, output_e2, output_e3], 1)
        #output_e = linear(output_e, FLAGS.hidden_size_e, scope_name = "event_output_layer", reuse_scope=(i!=0))
        outputs_e.append(output_e)

    # trainable modulator
    norm_c = tf.random_normal_initializer(stddev=1.0, dtype=tf.float32)
    with tf.variable_scope("modulator_e", reuse=tf.AUTO_REUSE) as scope: 
        inputs_a = tf.get_variable('a', [FLAGS.hidden_size_e0 + FLAGS.hidden_size_e1 + FLAGS.hidden_size_e2 + FLAGS.hidden_size_e3], initializer=norm_c, dtype=tf.float32)
        outputs_A = []
        for i in range(FLAGS.num_unroll_step):
            outputs_A.append(inputs_a)


    # the enhanced vector representation for event_level entity by memory block
    with tf.variable_scope("Enhance_Representation") as scope_name:
        outputs_enhanced = []
        for i, (output_a, output_e) in enumerate(zip(outputs_A, outputs_e)):
            if i > 0: scope.reuse_variables()
            outputs_enhanced.append(tf.multiply(output_a, output_e))
    ...
    with tf.variable_scope("lstm2") as scope:
        ...
        outputs_2 = []
        states_2 = []
        for i,input_ in enumerate(outputs_enhanced):
            if i > 0: scope.reuse_variables()
    ...
    output = tf.reshape(outputs_concat, [-1, FLAGS.hidden_size_2])
    print("----------output-------------")
    ...
    for i in range(target_vocabulary_num):
        logits = linear(output, target_vocabulary[i], scope_name='output_logits'+str(i), reuse_scope = False)
    ...
    cost = tf.reduce_sum(losses[0]+ losses[1]+losses[2]+ losses[3] , name="SumLoss") / FLAGS.batch_size
    ...
    for i in range(FLAGS.num_epochs):
        ...
        for batchNum, (c, x, y) in enumerate(
            ...
            batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, 
                crossEntropys, cost, finalState, train_op], {input_data_c0: c[0],input_data_e0: x[0], input_data_e1: x[1],
                input_data_e2: x[2],input_data_e3: x[3], targets[0]:y[0], targets[1]:y[1], targets[2]:y[2],targets[3]:y[3]})
    ...
    saver.save(sess, FLAGS.traindir)
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, crossEntropys, cost, finalState, tf.no_op()], {
            input_data_c0: c[0],input_data_e0: x[0], input_data_e1: x[1],input_data_e2: x[2],input_data_e3: x[3], targets[0]:y[0], targets[1]:y[1], targets[2]:y[2],targets[3]:y[3]})
        accumCosts += batchCost
        accumNumCorrPred = plus(accumNumCorrPred, batchNumCorrPred)
        accumCrossEnt = plus(accumCrossEnt, batchCrossEnt)
        iters += FLAGS.num_unroll_step
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "HelpDesk/SuffixPrediction{0}Dataset.txt".format(dataset), "w")
        ...
        for batchNum in range(numBatches):
            ...
            for i in range(maxSuffixLen):
                batchPredictions, batchsoftmaxes, batchFinalState, _ = sess.run([prediction_indices, softmaxes, finalState, tf.no_op()], {
                    input_data_c0: c[0],input_data_e0: x[0], input_data_e1: x[1], input_data_e2: x[2],input_data_e3: x[3],
                    targets[0]: y, targets[1]: y, targets[2]: y,targets[3]: y})
            ...
            suffixes_alpha = [''.join([num2alpha[element] for element in suffix]) for suffix in suffixes[0]]
            for j in range(FLAGS.batch_size):
                outputFile.write("{0}\n".format(suffixes_predicted_alpha[j]))
                outputFile.write("{0}\n".format(suffixes_alpha[j]))
                distance = dl.normalized_damerau_levenshtein_distance(suffixes_predicted_alpha[j], suffixes_alpha[j])
                outputFile.write("{0}\n".format(distance))
