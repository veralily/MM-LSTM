...
flags.DEFINE_string("traindir", "./train/2013", "Directory to save checkpoints and gnuplot files.")
...
flags.DEFINE_integer("num_epochs_full_lr", 30,
                     "The number of epochs with full learning rate.")
flags.DEFINE_integer("num_epochs", 50, "The exact number of epochs.")
...
resultFile = open("2013/ResultFile_withattribute" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".logging.csv", 'w')
...
dataset = "BPIC2013_withattribute.txt"
...
with tf.Graph().as_default() as G:
    ...
    with tf.variable_scope("lstm_event_e3") as scope:
        ...
        outputs_e3 = []
        state = initialState
        for i,input_ in enumerate(input_es[3]):
            if i > 0: scope.reuse_variables()            
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_e3,
                                scope_name='input_layer', reuse_scope=(i!=0)))
    ...
    with tf.variable_scope("lstm_event_e4") as scope:
        # Create the individual LSTM layers
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_e4, forget_bias=0.0)
        # Add a propabilistic dropout to cells of the LSTM layer
        if (FLAGS.dropout_prob > 0):
            lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=1.0-FLAGS.dropout_prob)
        # Replicate this (including dropout) to additional layers
        ...
        outputs_e4 = []
        state = initialState
        for i,input_ in enumerate(input_es[4]):
            if i > 0: scope.reuse_variables()            
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_e4,
                                scope_name='input_layer', reuse_scope=(i!=0)))
            
            output, state = cell(input_, state)
            print("ouput.shape,{}".format(output.get_shape()))
            outputs_e4.append(output)

        # connect inputs and outputs to the RNN layers
        print("----------outpute3-------------")
        print(outputs_e4[0].get_shape())
        print("------------------------------------")

    with tf.variable_scope("lstm_event_e5") as scope:
        # Create the individual LSTM layers
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_e5, forget_bias=0.0)
        # Add a propabilistic dropout to cells of the LSTM layer
        if (FLAGS.dropout_prob > 0):
            lstm_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=1.0-FLAGS.dropout_prob)
        # Replicate this (including dropout) to additional layers
        ...
        outputs_e5 = []
        state = initialState
        for i,input_ in enumerate(input_es[5]):
            if i > 0: scope.reuse_variables()            
            input_ = tf.nn.relu(linear(input_, FLAGS.hidden_size_e5,
                                scope_name='input_layer', reuse_scope=(i!=0)))
            
            output, state = cell(input_, state)
            print("ouput.shape,{}".format(output.get_shape()))
            outputs_e5.append(output)

        # connect inputs and outputs to the RNN layers
        print("----------outpute3-------------")
        print(outputs_e5[0].get_shape())
        print("------------------------------------")


    outputs_e = []
    for i, (output_e0, output_e1, output_e2, output_e3, output_e4, output_e5) in enumerate(zip(outputs_e0, 
        outputs_e1, outputs_e2, outputs_e3,outputs_e4, outputs_e5)):
        if i > 0: scope.reuse_variables()
        output_e = tf.concat([output_e0, output_e1, output_e2, output_e3, output_e4, output_e5], 1)
        #output_e = linear(output_e, FLAGS.hidden_size_e, scope_name = "event_output_layer", reuse_scope=(i!=0))
        outputs_e.append(output_e)

    # the trainable modulator
    norm_c = tf.random_normal_initializer(stddev=1.0, dtype=tf.float32)
    with tf.variable_scope("modulator", reuse=tf.AUTO_REUSE) as scope: 
        inputs_c = tf.get_variable('c', [FLAGS.hidden_size_e0 + FLAGS.hidden_size_e1 + FLAGS.hidden_size_e2 + FLAGS.hidden_size_e3 + FLAGS.hidden_size_e4 + FLAGS.hidden_size_e5], initializer=norm_c, dtype=tf.float32)
        outputs_A = []
        for i in range(FLAGS.num_unroll_step):
            outputs_A.append(inputs_c)
    ...
    cost = tf.reduce_sum(losses[0]+losses[1]+losses[2]+losses[3]+losses[4]+losses[5] , name="SumLoss") / FLAGS.batch_size
    ...
    for i in range(FLAGS.num_epochs):
        ...
        for batchNum, (c, x, y) in enumerate(
            ...
            batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, 
                crossEntropys, cost, finalState, train_op], {input_data_c0: c[0],input_data_es[0]: x[0], input_data_es[1]: x[1],
                input_data_es[2]: x[2],input_data_es[3]: x[3], input_data_es[4]: x[4], input_data_es[5]: x[5], 
                targets[0]:y[0], targets[1]:y[1], targets[2]:y[2],targets[3]:y[3] ,targets[4]:y[4],targets[5]:y[5]})
    ...
    for batchNum, (c, x, y) in enumerate(read.words_iterator(input_validation_traces, target_validation_traces, FLAGS.batch_size, FLAGS.num_unroll_step)):
        batchNumCorrPred, batchCrossEnt, batchCost, state, _ = sess.run([numCorrectPredictions, crossEntropys, cost, finalState, tf.no_op()], {
            input_data_c0: c[0],input_data_es[0]: x[0], input_data_es[1]: x[1],input_data_es[2]: x[2],input_data_es[3]: x[3], input_data_es[4]: x[4], input_data_es[5]: x[5], 
            targets[0]:y[0], targets[1]:y[1], targets[2]:y[2],targets[3]:y[3],targets[4]:y[4],targets[5]:y[5]})
        accumCosts += batchCost
        accumNumCorrPred = plus(accumNumCorrPred, batchNumCorrPred)
        accumCrossEnt = plus(accumCrossEnt, batchCrossEnt)
    ...
    if (targetVocabulary==inputVocabulary):
        outputFile = open(
            "2013/SuffixPrediction{0}Dataset.txt".format(dataset), "w")
        ...
        for batchNum in range(numBatches):
            ...
            for i in range(maxSuffixLen):
                batchPredictions, batchsoftmaxes, batchFinalState, _ = sess.run([prediction_indices, softmaxes, finalState, tf.no_op()], {
                    input_data_c0: c[0],input_data_es[0]: x[0], input_data_es[1]: x[1], input_data_es[2]: x[2],input_data_es[3]: x[3], input_data_es[4]: x[4],input_data_es[5]: x[5],
                    targets[0]: y, targets[1]: y, targets[2]: y,targets[3]: y,targets[4]:y,targets[5]:y})
